...
export function TaskItem({ task, onEdit, onDelete, onComplete }: TaskItemProps) {
    ...
    return (
        <Card className={cn("mb-3 transition-all hover:shadow-md", task.status === 'done' && "opacity-75")}>
            <CardContent className="p-4 flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1">
                    <Button
                        variant="ghost"
                        size="icon"
                        className={cn("mt-1 h-6 w-6 rounded-full", task.status === 'done' ? "text-green-500" : "text-gray-400")}
