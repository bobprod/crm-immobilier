...
export function PropertyBulkActions({ selectedCount, onAction }: PropertyBulkActionsProps) {
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-primary/5 rounded-lg border border-primary/10 mb-4">
      <span className="text-sm font-medium px-2">{selectedCount} sélectionné(s)</span>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => onAction('status', 'available')}>
            Disponible
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onAction('status', 'reserved')}>
            Réservé
          </DropdownMenuItem>
